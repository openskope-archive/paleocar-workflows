...
FedData::download_data(
  "http://nrdata.nps.gov/programs/Lands/meve_tracts.zip", 
  destdir="./data-raw/MVNP"
)

# uncompress shapefile directory
utils::unzip(
  "./data-raw/MVNP/meve_tracts.zip", 
  exdir="./data-raw/MVNP/meve_tracts"
)

# read shapefile
mvnp <- rgdal::readOGR(
  "./data-raw/MVNP/meve_tracts",
  layer='MEVE_boundary'
)
...
## Set a spatial buffer around the area to reconstruct
tree.buffer <- 10

## Get Tree-ring data from the ITRDB for 10-degree buffer around MVNP
treePoly <- suppressWarnings(rgeos::gBuffer(
  mvnp, 
  width=tree.buffer,
  quadsegs=1000
))
...
itrdb <- FedData::get_itrdb(
  template=treePoly,
  label="MVNP_PLUS_10DEG", 
  raw.dir="./data-raw/ITRDB/RAW/ITRDB/", 
  extraction.dir="./data-raw/ITRDB/EXTRACTIONS/ITRDB/", 
  recon.years=prediction.years,
  calib.years=calibration.years,
...
#######################################################
##### ACQUIRE PRISM WATER-YEAR PRECIPITATION DATA #####
#######################################################
...
mvnp_prism_ppt_monthly <- raster::brick(
  "./data-raw/mvnp_prism_ppt_monthly.nc4"
)

mvnp_prism <- annualize_prism_monthly(
  prism.brick=mvnp_prism_ppt_monthly, 
  months=c(-2:9), 
  fun='sum'
)
